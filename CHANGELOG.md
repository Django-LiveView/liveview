...
### Added
- **Script cleanup**: `el.__cleanup()` is called automatically before replacing or removing an element, allowing scripts to release timers, observers, and third-party library instances (e.g. `chart.destroy()`)
- **Script local context**: inline `<script>` tags in HTML responses now receive `el` and `this` bound to the target element, removing the need for global DOM selectors

### Changed
- Script extraction now uses DOMParser instead of regex, handling edge cases such as template literals containing `</script>`
- Scripts with a non-executable `type` attribute (e.g. `text/template`) are no longer extracted or executed
